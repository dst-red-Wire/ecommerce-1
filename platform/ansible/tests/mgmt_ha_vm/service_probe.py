...
def dns_query(server: str, name: str, timeout: float = 3.0) -> str:
    ident = 0x4841
...
def ntp_query(server: str, timeout: float = 3.0) -> float:
...
def main() -> int:
    ...
    actual = dns_query(str(ipaddress.IPv4Address(args.dns_server)), args.dns_name)
    if actual != expected:
        raise SystemExit(f"DNS answer mismatch: expected {expected}, got {actual}")
    offset = ntp_query(str(ipaddress.IPv4Address(args.ntp_server)))
    if abs(offset) > args.max_ntp_offset:
        raise SystemExit(f"NTP offset exceeds bound: {offset:.3f}s")
    print(json.dumps({
        "dns": {"name": args.dns_name, "address": actual, "server": args.dns_server},
        "ntp": {"server": args.ntp_server, "offset_seconds": round(offset, 6)},
    }, sort_keys=True))
    return 0
