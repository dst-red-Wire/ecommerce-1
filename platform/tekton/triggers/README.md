...
Run `make tekton-trigger-readiness RUNTIME_CONFIG=<path>` to collect those proofs without mutating the cluster. The runtime document must include `execution_budget.max_workers` (1..16); this measured value is injected into every affected/global PipelineRun, while the repository's worker default applies only to local qualification. The gate renders `platform/tekton` with Kustomize, then uses read-only `kubectl` calls against the explicit management-plane context. Helm remains available for chart-owned components but is not invoked merely as a second renderer when no chart owns the trigger resources. A missing proof returns `BLOCKED`, never a fabricated PASS.
