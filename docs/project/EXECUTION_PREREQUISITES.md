...
Status: `ACTIVE — ARCHITECTURE INPUTS COMPLETE`
...
- deterministic network/IPAM contract;
- storage topology/device ownership;
- service ownership matrix;
- data ownership matrix;
- event contract matrix;
- security trust zones;
- deployment DAG;
- machine-readable PREPROD/PROD inventories;
- machine-readable network plan;
...
Architecture ambiguity is not an accepted routine blocker after these contracts merge. A future `BLOCKED_ARCHITECTURE` must point to a concrete contradiction or new structural requirement, not a missing description of the current baseline.
...
| Hetzner account, final orderable PROD hardware matching or exceeding certified BOM and network allocation | M8 cert/M9 | business owner/provider | M8/M9 |
...
ChatGPT selects architectural products and exact topology; Codex resolves implementation-compatible current versions in a versioned lock/BOM after compatibility checks.
...
These are engineering deliverables owned by Codex, not architecture questions.
...
- concrete contradiction/new structural requirement -> `BLOCKED_ARCHITECTURE` and route to ChatGPT;
- failing implementation/test/version compatibility -> `BLOCKED_ENGINEERING` and keep with Codex;
