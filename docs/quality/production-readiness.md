...
| Staging | full integration/API/authorization, DAST, identity/fraud/abuse, resilience, performance, load/stress as scheduled | Authorized and bounded |
...
`tests/production-readiness/gates.json`. Every production gate requires a JSON
result at `artifacts/gates/<gate-id>.json` containing at least
`{"status":"pass"}`. Generated reports are ignored by Git and must be retained
by the CI artifact store.
...
- Tests use synthetic accounts and data in an authorized staging environment.
