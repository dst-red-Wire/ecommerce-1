...
`developer_toolchain` and `developer_workstation` are workstation roles and are not Docker Molecule candidates: they require the supported Ubuntu WSL2 host, user systemd and rootless Docker lifecycle controls. The container-runtime parity gate exercises the exact rootless Engine, Testcontainers PostgreSQL/Ryuk cleanup and a single-node `kind` smoke before Docker Desktop may be retired. `rocky_baseline` manages `chronyd` through systemd; it should receive a Rocky 9 systemd-capable Molecule scenario only when the chosen runner can provide real cgroups and service management.
