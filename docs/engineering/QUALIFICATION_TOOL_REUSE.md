...
Seed validation first reconstructs the installed file inventory from retained wheels
whose SHA-256 matches `requirements.lock`; the interpreter's ensurepip wheels cover pip.
Wheel tags must also match the running interpreter and platform. Invalid references are
replaced as a complete directory, with rollback if publication fails; a complete trusted
subset can be recovered offline from a reference containing extra or duplicate wheels.
Missing or invalid wheel references refuse reuse. Cold preparation acquires binary wheels
with hash enforcement; warm verification and repairs with complete references are offline.
Wheel payloads and metadata must match byte-for-byte. Generated entry points are derived
with pip's installer-specific script transformation and require execution permission on
POSIX. Legitimate venv aliases include Python 3.14's UTF-8 `𝜋thon` alias.
INSTALLER/REQUESTED have fixed content, RECORD must
list the authenticated files and hashes, and generated bytecode must match compilation
of authenticated source. Unexpected site-packages files and symlinks are rejected.
This is corruption detection rooted in the repository lock and trusted interpreter,
not isolation from a hostile process able to replace that interpreter or repository.
Validation then evaluates PEP 508 markers using pip's vendored parser, checks exact
installed versions and `pip check`. Simulated
