...
Roadmap milestone status follows the same rule. `config/contracts/roadmap-policy.yaml` declares dependencies, implementation requirements, qualification gates, QCE capability mappings, and runtime-evidence requirements; it contains no manually maintained completion result. `make roadmap-check` writes the non-authoritative machine projection to `.context/evidence/roadmap-status.json`. A completed tracker or existing manifest can establish progress, but only canonical exact-SHA qualification evidence and every required, fresh, environment-bound runtime proof can establish `PROVEN`. An unmet milestone dependency or a mapped `BLOCKED` QCE capability fails closed. The versioned execution-plan table therefore points to the derived command instead of embedding a self-invalidating proof snapshot.
