...
class ProductGoldenServiceContractTest(unittest.TestCase):
    def test_product_event_names_match_canonical_registry(self) -> None:
        registry = yaml.safe_load((ROOT / "config/contracts/event-contracts.yaml").read_text(encoding="utf-8"))
        source = (ROOT / "services/product/internal/application/service.go").read_text(encoding="utf-8")
        expected = {
    ...
    def test_fleet_chart_renders_restricted_digest_only_workload(self) -> None:
        ...
        rendered = subprocess.run(command, cwd=ROOT, check=True, text=True, capture_output=True).stdout
        documents = [document for document in yaml.safe_load_all(rendered) if document]
        by_kind = {document["kind"]: document for document in documents if document["kind"] != "NetworkPolicy"}
        self.assertTrue({"Namespace", "ServiceAccount", "ConfigMap", "Deployment", "Service", "HorizontalPodAutoscaler", "PodDisruptionBudget"}.issubset(by_kind))
        ...
        self.assertEqual(container["image"], f"harbor.invalid/ecommerce/product@{digest}")
        ...
        default_deny = next(policy for policy in policies if policy["metadata"]["name"] == "product-default-deny")
    ...
    def test_tekton_product_release_is_supply_chain_only(self) -> None:
        ...
        source = pipeline_path.read_text(encoding="utf-8") + task_path.read_text(encoding="utf-8")

        self.assertEqual(pipeline["metadata"]["name"], "ecommerce-product-release")
        task_names = [task["name"] for task in pipeline["spec"]["tasks"]]
        self.assertEqual(task_names, ["source-checkout", "product-gates", "build-push", "scan-sbom", "sign-attest"])
