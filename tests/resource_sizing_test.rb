...
class ResourceSizingTest < Minitest::Test
  ...
  def test_candidate_is_deterministic_and_keeps_headroom
    ...
    assert_equal "preprod", result.fetch("source_environment")
  ...
  def test_non_preprod_evidence_fails_closed
    bad = evidence.merge("source_environment" => "development")
    error = assert_raises(ArgumentError) do
      ResourceSizing.candidate(bad, ResourceSizing.load_policy(ROOT))
    end
    assert_includes error.message, "source_environment must match policy source"
  end
