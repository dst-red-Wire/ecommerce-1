...
class SeedRepairReal(unittest.TestCase):
    def test_corrupt_active_seed_repairs_once_and_serializes_writers(self):
        with tempfile.TemporaryDirectory(prefix="pr86-seed-") as directory:
            ...
            cyclic_repair = invoke(sys.executable)
            self.assertEqual(0, cyclic_repair.returncode, cyclic_repair.stdout + cyclic_repair.stderr)
            self.assertIn("PREPARE qualification seed", cyclic_repair.stdout)
            self.assertNotEqual(old, reference.resolve())
            self.assertTrue(old.is_dir(), "cyclic-selector repair retains published readers")
            old = reference.resolve()
            reference.unlink()
