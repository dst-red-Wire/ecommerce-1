...
class GoToolchainDeliveryTests(unittest.TestCase):
    ...
    def test_publish_uses_affected_exact_sha_controller(self):
        ...
        publish = next(node for node in module.body if isinstance(node, ast.FunctionDef) and node.name == "publish")
        calls = [node for node in ast.walk(publish) if isinstance(node, ast.Call)]

        verify_calls = [call for call in calls if isinstance(call.func, ast.Name) and call.func.id == "verify_change"]
        self.assertTrue(
            any(
                len(call.args) >= 2
                and isinstance(call.args[0], ast.Name)
                and call.args[0].id == "base_ref"
                and isinstance(call.args[1], ast.Name)
                and call.args[1].id == "head"
