...
test("Admin: navigation produit, filtre et édition simulée", async ({ page }) => {
...
test("Admin: stock et remboursement restent explicitement simulés", async ({ page }) => {
  await page.goto("http://127.0.0.1:3001/inventory");
  await expect(page.getByRole("heading", { name: "Ajuster le stock" })).toBeVisible();
  await page.getByRole("button", { name: "Confirmer l’ajustement" }).click();
  await expect(page.getByRole("status")).toContainText("aucune donnée backend modifiée");

  await page.goto("http://127.0.0.1:3001/payments/PAY-2026-008471");
  const dialog = page.getByRole("dialog");
  await expect(dialog.getByRole("heading", { name: "Rembourser le paiement" })).toBeVisible();
  ...
  await expect(dialog.getByRole("button", { name: "Confirmer le remboursement" })).toBeEnabled();
});

test("Admin: order et payment inconnus renvoient 404", async ({ page }) => {
  const orderResponse = await page.goto("http://127.0.0.1:3001/orders/ORD-UNKNOWN");
  expect(orderResponse?.status()).toBe(404);
  await expect(page.getByRole("heading", { name: "Vue non implémentée" })).toBeVisible();

  const paymentResponse = await page.goto("http://127.0.0.1:3001/payments/PAY-UNKNOWN");
...
test("Admin: vue mobile d’urgence et acquittement", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Urgences opérationnelles" })).toBeVisible();
  await page.getByRole("button", { name: "Accuser réception" }).click();
  await expect(page.getByRole("button", { name: /Accusé réception/ })).toBeDisabled();
});
