...
def validate_image_archive(path: Path) -> list[str]:
    ...
    # Python, before any package transaction. Nested OCI layers are not extracted.
    with tarfile.open(path, mode="r:") as archive:
        seen = set()
        identities, references, regular_members = [], [], set()
        for member in archive:
            ...
            seen.add(str(name))
            if member.isfile():
                regular_members.add(str(name))
            if str(name) in {"manifest.json", "index.json"}:
                require(member.isfile() and 0 < member.size <= 8 * 1024 * 1024, "invalid image index size")
                stream = archive.extractfile(member)
                require(stream is not None, "missing image index")
                with stream:
                ...
                if str(name) == "manifest.json":
                    ...
                    for image in index:
                        ...
                        require(isinstance(config, str) and isinstance(layers, list) and layers
                                and all(isinstance(layer, str) for layer in layers), "image config and layer inventory required")
                        references.extend([config, *layers])
                        tags = image.get("RepoTags")
                        require(isinstance(tags, list) and tags, "image identities missing")
                        for tag in tags:
                            require(isinstance(tag, str) and ":" in tag and not tag.endswith(":latest"), "unpinned image identity")
                    ...
                    for descriptor in descriptors:
                        require(isinstance(descriptor, dict), "invalid OCI image descriptor")
                        ...
                        references.append("blobs/sha256/" + identity.split(":", 1)[1])
        require(identities, "image archive lacks Docker or OCI manifest")
        require(all(str(PurePosixPath(reference)) in regular_members for reference in references),
    ...
    for entry in artifacts:
        ...
        if category == "rpm":
            ...
            if rpm_metadata:
                ...
                require(metadata.stdout == nevra, "RPM metadata differs from approved NEVRA")
            if rpm_signatures:
                signature = subprocess.run(["rpm", "--checksig", "--verbose", str(path)],
                                           check=True, capture_output=True, text=True, timeout=30)
                require(re.search(r"(?m)^.*Signature.*: OK$", signature.stdout), "RPM signature verification required")
            rpms.append(name)
