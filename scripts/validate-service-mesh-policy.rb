...
POLICY_PATH = File.join(ROOT, "config/contracts/service-mesh-policy.yaml")

def load_yaml(path)
  YAML.safe_load(File.read(path), aliases: false) || {}
rescue Psych::Exception => e
  warn "FAIL #{File.basename(path)} invalid YAML: #{e.message}"
  exit 1
end
  ...
  if waypoint_required
    ...
    justifications.each_with_index do |item, index|
      ...
      %w[owner capability reason source_contract].each do |field|
        value = item[field]
        errors << "#{service}: justification[#{index}].#{field} must be a non-empty string" unless value.is_a?(String) && !value.strip.empty?
      end
